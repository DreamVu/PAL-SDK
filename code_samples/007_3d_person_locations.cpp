...
CODE SAMPLE # 007: 3D Person Location
This code sample allows users to run Person Detection and find the 3D location of each detected person. 


>>>>>> Compile this code using the following command....

g++  007_3d_person_locations.cpp /usr/src/tensorrt/bin/common/logger.o ../lib/libPAL.so ../lib/libPAL_CAMERA.so  ../lib/libPAL_DEPTH_HQ.so ../lib/libPAL_DEPTH_128.so  ../lib/libPAL_DE.so ../lib/libPAL_EDET.so `pkg-config --libs --cflags opencv`   -O3  -o  007_3d_person_locations.out -I../include/ -lv4l2 -lpthread -lcudart -L/usr/local/cuda/lib64 -lnvinfer -lnvvpi -lnvparsers -lnvinfer_plugin -lnvonnxparser -lmyelin -lnvrtc -lcudart -lcublas -lcudnn -lrt -ldl -lstdc++fs -lX11


>>>>>> Execute the binary file by typing the following command...

./007_3d_person_locations.out
...
{
	namedWindow( "PAL 3D Person Locations", WINDOW_NORMAL ); // Create a window for display.
	...
	{
		printf("3D Person Locations Init failed\n");
		return 1;
	}
	...
	resizeWindow("PAL 3D Person Locations", sc_width, sc_height);//width/4, (height/4)*2);
	int key = ' ';
	...
	while(key != 27)
		...
		PAL::GetPeopleDetection(rgb,right, depth, &Boxes, DepthValues, &Loc3Ds, NULL);
		...
		for(int i =0; i<num_of_persons; i++)
		{
		    ...
		    if(DepthValues[i] > 3)
		    {
                cv::circle(rgb, Point((Boxes[i].x1+Boxes[i].x2)/2, (Boxes[i].y1+Boxes[i].y2)/2), 5, cv::Scalar(0,255,0), -1);
                setLabel(rgb, text, Point((Boxes[i].x1+Boxes[i].x2)/2, (Boxes[i].y1+Boxes[i].y2)/2-10), CV_RGB(0,255,0));
		    }
		    else
		    {
                cv::circle(rgb, Point((Boxes[i].x1+Boxes[i].x2)/2, (Boxes[i].y1+Boxes[i].y2)/2), 5, cv::Scalar(0,0,255), -1);
                setLabel(rgb, text, Point((Boxes[i].x1+Boxes[i].x2)/2, (Boxes[i].y1+Boxes[i].y2)/2-10), CV_RGB(255,0,0));
            }
		}
		...
		imshow( "PAL 3D Person Locations", rgb); 
