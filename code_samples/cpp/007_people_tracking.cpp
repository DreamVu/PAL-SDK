...
void drawOnImage(cv::Mat &img, const PAL::Data::TrackingResults &data,
    bool ENABLEDEPTH=false, bool ENABLE3D=false)
{
    ...
    //bool only_detection = (mode == PAL::Mode::OBJECT_DETECTION) ? true : false;
...
int main( int argc, char** argv )
{
    ...
    //prop.start_vfov = 53;
    //unsigned long int flags = PAL::STARTVFOV;
    ...
    double t1 = (double)(start_time.tv_sec*1000+start_time.tv_usec/1000);
	...
	while(key != 27)
	{
        ...
        drawOnImage(display, data[0], enableDepth, enable3Dlocation);

        

        /*
        cv::Mat depth = data[0].depth.clone();
        depth.convertTo(depth, CV_8UC1);
        cvtColor(depth, depth, cv::COLOR_GRAY2BGR);

        
        if(depth.size() != display.size())
        {
            std::cout << "depth size: " << depth.size() << std::endl;
            cv::resize(depth, depth, display.size());
        }

        //vconcat(data[0].stereo_left, data[0].stereo_right, display);
        vconcat(display, depth, display);
        */
        
		//Display the stereo images
		imshow( "PAL People Tracking", display);  
        ...
        if(k == 20)
        {
            ...
            double t2 = (double)(current_time.tv_sec*1000 + current_time.tv_usec/1000);
            fps = 20*1000/(t2-t1);
            cout << "fps: " << fps << endl;
            t1 = t2;
            k=0;
	...
	if (key == 'v' || key == 'V')
        ...
        if (key == 's' || key == 'S')
        {
            PAL::SaveProperties("../../../Explorer/SavedPalProperties.txt");
        }
	}
