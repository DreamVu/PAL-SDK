...
namespace PAL
{
	...
	*/
	...
	PAL::Acknowledgement GetPointCloud(std::vector<PAL::Point> *pc, timeval *timestamp = 0,
		PAL::Image *left = 0, PAL::Image *right = 0, PAL::Image *depth = 0);
	...
	//threshold	: Confidence threshold of the detections ranging from 0 to 1 
    	PAL::Acknowledgement InitPersonDetection(float threshold = 0.4);
	...
	PAL::Acknowledgement GetPeopleDetection(cv::Mat& rgb_l, cv::Mat& rgb_r, cv::Mat& depth,    
		                            std::vector<PAL::BoundingBox> *BoundingBoxes, std::vector<float> &DepthValues, std::vector<PAL::Loc3D>* Loc3Ds = 0,
		                            timeval *timestamp = 0);
	
	PAL::Acknowledgement GetAllDetection(cv::Mat& rgb_l, cv::Mat& rgb_r, cv::Mat& depth,    
                                            std::vector<std::pair<int, PAL::BoundingBox>> *BoundingBoxes, std::vector<float> &DepthValues, std::vector<PAL::Loc3D>* Loc3Ds=0,
                                            timeval *timestamp=0);	
