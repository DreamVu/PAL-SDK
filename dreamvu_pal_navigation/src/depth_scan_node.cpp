...
static void RunThread()
{
	...
	while(g_bRosOK)
	{
		...
		ros::WallTime start = ros::WallTime::now();
  		...
  		if(pointcloudSubnumber > 0 && updatePC)
		{
...
int main(int argc, char **argv)
{
	...
	while (g_bRosOK)
	{
		...
		{
            publishimage(data1[0].stereo_left, stereoleftpub1, "bgr8", data1[0].timestamp);
            g_emode = g_emode | PAL::API_Mode::STEREO;	            	
        }        
		if (stereoright1Subnumber > 0)
		{				
            publishimage(data1[0].stereo_right, stereorightpub1, "bgr8", data1[0].timestamp);	
            g_emode = g_emode | PAL::API_Mode::STEREO;	            	
